...
#define MF_ID           (0x1F)
#define GD_ID           (0x04)
#elif(SPI_FLASH_TYPE == IS25LPXX) 
#define MF_ID           (0x9D)
#define GD_ID           (0x60)
#endif

/* JEDEC Device ID: Memory type and Capacity */
#define MTC_AT25SF041A              (0x04) /* AT25SF041A */
#define MTC_IS25LP064A				(0x16)	/*IS25LP064A 64M bits*/
#define MTC_IS25LP128A				(0x17)	/*IS25LP064A 128M bits*/
...
static uint8_t at25sfxx_read_status(void)
{
    return rt_spi_sendrecv8(spi_flash_device.rt_spi_device, CMD_RDSR1);
}

static void at25sfxx_wait_busy(void)
{
	/*may be endless loop, need to do, rch 2017.08.31*/
    while( at25sfxx_read_status() & (0x01));
}
 ...
 *
 */
static uint32_t at25sfxx_read(uint32_t offset, uint8_t * buffer, uint32_t size)
{
 ...
 *
 */
uint32_t at25sfxx_page_write(uint32_t page_addr, const uint8_t* buffer)
{
    uint32_t index;
    uint8_t send_buffer[4];
    ...
    at25sfxx_wait_busy(); // wait erase done.
...
static rt_err_t at25sfxx_flash_init(rt_device_t dev)
{
    return RT_EOK;
}

static rt_err_t at25sfxx_flash_open(rt_device_t dev, rt_uint16_t oflag)
{
...
static rt_err_t at25sfxx_flash_close(rt_device_t dev)
{
    return RT_EOK;
}

static rt_err_t at25sfxx_flash_control(rt_device_t dev, rt_uint8_t cmd, void *args)
{
...
static rt_size_t at25sfxx_flash_read(rt_device_t dev,
                                   rt_off_t pos,
                                   void* buffer,
                                   rt_size_t size)
{
    ...
    at25sfxx_read(pos*spi_flash_device.geometry.bytes_per_sector,
                buffer,
...
static rt_size_t at25sfxx_flash_write(rt_device_t dev,
                                    rt_off_t pos,
                                    const void* buffer,
                                    rt_size_t size)
{
    ...
    while(block--)
    {
        at25sfxx_page_write((pos + i)*spi_flash_device.geometry.bytes_per_sector,
                          ptr);
        ptr += PAGE_SIZE;
        i++;
    }
...
rt_err_t at25sfxx_init(const char * flash_device_name, const char * spi_device_name)
{
    ...
    /* config spi */
    {
        ...
        cfg.max_hz     = SPI_FLASH_NRD_MAXMZ * 1000 * 1000; /* 50M */
    ...
    /* init flash */
    {
        ...
        id_recv[0] = 0xFF; /* reset SPI FLASH, cancel all cmd in processing. */
			  id_recv[1] = 0xFF;
        rt_spi_send(spi_flash_device.rt_spi_device, id_recv, 2);
...
//for(;;)			
...
#if(SPI_FLASH_TYPE == AT25SFXX)
        if(id_recv[0] != MF_ID && id_recv[1] >> 5 != GD_ID)
#elif(SPI_FLASH_TYPE == IS25LPXX)
        if(id_recv[0] != MF_ID && id_recv[1] != GD_ID)
#endif
        {
...
#if(SPI_FLASH_TYPE == AT25SFXX)
        memory_type_capacity = id_recv[1] & 0x1F;
#elif(SPI_FLASH_TYPE == IS25LPXX)
        memory_type_capacity = id_recv[2];
#endif

        FLASH_TRACE("cpacity %04X\r\n", memory_type_capacity);
		
#if(SPI_FLASH_TYPE == AT25SFXX)
        if(memory_type_capacity == MTC_AT25SF041A)
#elif(SPI_FLASH_TYPE == IS25LPXX)
        if(memory_type_capacity == MTC_IS25LP064A ||memory_type_capacity == MTC_IS25LP128A)
#endif
        {
            FLASH_TRACE("AT25SF041A detection\r\n");
    ...
    /* register device */
    spi_flash_device.flash_device.type    = RT_Device_Class_SPIDevice;
    spi_flash_device.flash_device.init    = at25sfxx_flash_init;
    spi_flash_device.flash_device.open    = at25sfxx_flash_open;
    spi_flash_device.flash_device.close   = at25sfxx_flash_close;
    spi_flash_device.flash_device.read    = at25sfxx_flash_read;
    spi_flash_device.flash_device.write   = at25sfxx_flash_write;
    spi_flash_device.flash_device.control = at25sfxx_flash_control;
    /* no private */
