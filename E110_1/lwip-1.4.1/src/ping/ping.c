...
static void ping_recv(int s,rt_uint32_t *recv_time)
{
    char buf[64];
    int fromlen, len;
...
rt_err_t ping(char* target, rt_uint32_t time, rt_size_t size,int *suc_rate)
{
	...
	{
		/* 返回成功率 */
		*suc_rate = recv_time * 100 / send_time;
	}
    lwip_close(s);
