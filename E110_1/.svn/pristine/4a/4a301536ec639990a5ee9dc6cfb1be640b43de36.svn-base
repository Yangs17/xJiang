...
static void initAboxInformation()
{
	...
	aboxInfo.cboxDataPool = InitDataPool(CBOXDATAPOOLSIZE,"cbox_Poll");
	aboxInfo.subgDataPool = InitDataPool(SUBGDATAPOOLSIZE,"subg_Poll");
...
void cleanCbox()
{
	...
	{
		removeCbox(i);
		rt_memset(&aboxInfo.cboxTotalInfo.cboxInfo[i],0,sizeof(cbox_information));
		rt_thread_delay(100);
	}
...
//		if(cboxInfo_t->subgDataInfo.dataAddr >= SUBGDATAPOOLSIZE) return 0;
		if(cboxInfo_t->subgDataInfo.dataLength >= SUBGDATAPOOLSIZE) 
		{
			rt_memset(aboxInfo.subgDataPool->poolHead,0,SUBGDATAPOOLSIZE);
			aboxInfo.subgDataPool->entrySize = 0;
			
			rt_memset(&cboxInfo_t->subgDataInfo,0,sizeof(data_info));
			return 0;
		}
...
//			if(checkPack1.lenth) packnum++;
			...
			//warning:
			//wait for client send the data to panel.
			if(RT_EOK == rx_data(subgRecv, payload_length,ONCE_PACK_TIME_OUT*3))
			{			
...
/* 返回保存数据的长度 */
static rt_uint32_t getPoolDate(rt_uint8_t *saveBuf,cbox_data_info *cboxInfo_t)
{
	if(!saveBuf) return 0;
...
void traverseCbox()
{
	...
	{	
		...
		dataLen = getPoolDate(getData,&aboxInfo.cboxTotalInfo.cboxInfo[cboxIndex].cboxDataInfo);
		if(dataLen)
		{
...
void removeCbox(rt_uint8_t removeIndex)
{
	...
	rt_uint8_t tempBuff[BUFF_LEN]={0};
	...
	getPoolDate(tempBuff,&aboxInfo.cboxTotalInfo.cboxInfo[removeIndex].cboxDataInfo);
