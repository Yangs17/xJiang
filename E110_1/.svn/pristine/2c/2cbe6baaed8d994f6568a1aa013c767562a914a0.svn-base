...
int calc_img_crc(int fd, unsigned int *img_crc)    
{     
    int 					nread;    
...
rt_err_t calc_flash_crc(unsigned int addr,unsigned int size)
{
	unsigned int crc = 0xffffffff;
	...
	if(!crc) return RT_EOK;
	else	return RT_ERROR;
	
}

int	calc_file(char *filename)
{
		int						fd;
		unsigned int 	crc;	
    ...
    if (fd < 0)
    {
		...
		//rt_kprintf("fd open is ok fd = %d",fd);
		calc_img_crc(fd, &crc);
		 //rt_kprintf("calc_img_crc is ok\n");
		if (crc == 0)
		{	
				int		pos;
				crc = 0xffffffff;

				pos = lseek(fd, 0, DFS_SEEK_END);
				lseek(fd, pos - sizeof(unsigned int), DFS_SEEK_SET);					
				...
				rename(filename, "/firmware/rom.bin");	
		}
#if 1
		else
		{
				close(fd);
				rt_kprintf("crc file err\n");
				unlink(filename);
