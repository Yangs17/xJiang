...
rt_err_t handleSerNetConfigFile(serverNet_information *serNetConfig,char *fileBuf,int fileSize)
{
	...
	for(lineIndex=0;lineIndex<LINE_NUM_MAX;lineIndex++)
	{
		...
		if(lineIndex == WIFI_PASSWORD_LINE)
			break;
	}
...
/* open log file */
rt_err_t open_log_file(FIL *file_temp)
{
	u8 ret = 0;
	ret = f_open(file_temp, LOG_PRINTF_PATH,FA_CREATE_ALWAYS |FA_WRITE|FA_READ);
	if(ret == FR_OK)
		return RT_EOK;
	else
		return RT_ERROR;
...
void close_log_file(FIL *file_temp)
{
	f_close(file_temp);
}

rt_err_t write_log_file(FIL *file_t,char *write_buf,rt_uint32_t wlen)
{
	if(!file_t || !write_buf) return RT_ERROR;
	rt_uint32_t *bw = RT_NULL;
	if(FR_OK != f_write(file_t,write_buf,wlen,bw))
		return RT_ERROR;
...
//	u32 bw = 0;	
	...
	{	
		...
		{
			/* net set */
			ret = f_open(&file_temp, NET_SETTING,FA_OPEN_EXISTING |FA_READ);
			if(ret  == FR_OK)
			{ 			
			 	et = readNetSet(&file_temp,&serNetConfig);
				if(RT_EOK == et)
				{
				...
				{	
					rt_kprintf("net set error,err = %d!\r\n",et);
			...
			/* updata set */
			ret = f_open(&file_temp, APP_PROGRAM,FA_OPEN_EXISTING |FA_READ);
			if(ret  == FR_OK)
			{ 			
				if(RT_EOK == updataSet(&file_temp))
				{
			...
			/* test network's analysis */
			print_log_diagnosis();

			/* wait log setting ok! */
			wait_print_log();

			GPIO_ResetBits(LED_GAF, LED_MON);
		}		
