...
/* 不加延时，有可能读取usb 类出错 */ 
/* error reason:  USBH_MSC_UNRECOVERED_STATE; USBH_UNRECOVERED_ERROR;  */
if(phost->gState != HOST_IDLE)
rt_kprintf("host state:%d \r\n",phost->gState);  //添加延时，保证usb类的正确读取
  ...
  case HOST_IDLE :
	...
	rt_thread_delay(100);/* rtthread: thread exit() */
    break;
	

  case HOST_DEV_ATTACHED ://设备插入
    ...
    if ( HCD_ResetPort(pdev) == 0) //设备复位
    {
      ...
      */
      ...
      phost->usr_cb->DeviceSpeedDetected(phost->device_prop.speed);//获取从机速度
  ...
  case HOST_ENUMERATION:     
    ...
    { 
      ...
      phost->usr_cb->EnumerationDone(); //枚举
  ...
  case HOST_USR_INPUT:    
    ...
    if ( phost->usr_cb->UserInput() == USBH_USR_RESP_OK) //等待用户输入
    {
  ...
  case HOST_CLASS:   
    ...
    status = phost->class_cb->Machine(pdev, phost);// 进去对应的类
    USBH_ErrorHandle(phost, status);
    break;       
