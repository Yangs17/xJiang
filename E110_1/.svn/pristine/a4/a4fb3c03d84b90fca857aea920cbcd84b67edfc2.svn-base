...
//#else
	...
	//	{1000UL * 500,	MK_BKCAN_BAUD(CAN_SJW_2tq, CAN_BS1_9tq,  CAN_BS2_2tq, 6)},//36M
		...
		{1000UL * 250,	MK_BKCAN_BAUD(CAN_SJW_2tq, CAN_BS1_13tq, CAN_BS2_2tq, 9)},
...
static rt_err_t bxmodifyfilter(struct stm_bxcan *pbxcan, struct rt_can_filter_item *pitem, rt_uint32_t actived)
{
    ...
    {
        CAN_FilterInitStructure.CAN_FilterActivation = DISABLE;//关闭过滤器
    }
    if (!actived)
    {
        rt_enter_critical();
...
{
	//static rt_uint32_t last_err;
    rt_uint32_t errtype;
    errtype = CAN1->ESR;
	
    rt_interrupt_enter();

	//if((errtype & 0x70)!= last_err)
	//	rt_kprintf("can sce %x\n", errtype & 0x70);	

	//	last_err = (errtype & 0x70);
