...
//    LOG_W("Warning: tls uri, please enable mqtt tls support!");
  ...
  if (host_addr[0] == '[')
  {
    ...
    port_str[port_len] = '\0';
    rt_kprintf("ipv6 address port: %s", port_str);
  }
  else /* ipv4 or domain. */
  {
    ptr = strstr(host_addr, ":");
    ...
    port_str[port_len] = '\0';
    rt_kprintf("ipv4 address port: %s", port_str);
  }
  ...
  /* get host addr ok. */
  {
    ...
    rt_kprintf("HOST = '%s'", host_addr_new);
    ...
    if (ret != 0)
    {
      rt_kprintf("getaddrinfo err: %d '%s'", ret, host_addr_new);
      rc = -1;
      goto _exit;
    }
  }
...
static int mqtt_open_tls(mqtt_client *c)
{
  ...
  {
    rt_kprintf("open tls failed, no memory for tls_session buffer malloc");
    return -RT_ENOMEM;
  }
  ...
  {
    rt_kprintf("mbedtls_client_init err return : -0x%x", -tls_ret);
    return -RT_ERROR;
  }
...
static int net_connect(mqtt_client *c)
{
  ...
  if (c->tls_session)
  {
    ...
    {
      rt_kprintf("mbedtls_client_context err return : -0x%x", -tls_ret);
      return -RT_ERROR;
    }
    
    if ((tls_ret = mbedtls_client_connect(c->tls_session)) < 0)
    {
      rt_kprintf("mbedtls_client_connect err return : -0x%x", -tls_ret);
      rc = -RT_ERROR;
      goto _exit;
    }
    rt_kprintf("tls connect success...");
...
static int mqtt_connect(mqtt_client *c)
{
  ...
  
  {
    ...
    if (res <= 0)
    {
      rt_kprintf("%s wait resp fail, res:%d errno:%d", __FUNCTION__, res, errno);
      rc = -1;
      goto __connect_exit;
    }
  }
  ...
  if (rc < 0)
  {
    rt_kprintf("%s MQTTPacket_readPacket fail", __FUNCTION__);
    goto __connect_exit;
  }
...
static int mqtt_subscribe(mqtt_client *c, const char *topicFilter, enum QoS qos)
{
  ...
  if (rc < 0)
  {
    rt_kprintf("mqtt_subscribe MQTTPacket_readPacket mqtt_connect fail");
    goto _exit;
  }
...
__mqtt_start:
  ...
  if (rc != 0)
  {
    rt_kprintf("Net connect error(%d).", rc);
    goto __mqtt_restart;
  }
  ...
  if (rc != 0)
  {
    rt_kprintf("MQTT connect error(%d): %s.", rc, MQTTSerialize_connack_string(rc));
    goto __mqtt_restart;
  }
  
  rt_kprintf("MQTT server connect success.");
  ...
  for (i = 0; i < MAX_MESSAGE_HANDLERS; i++)
  {
    ...
    rt_kprintf("Subscribe #%d %s %s!", i, topic, (rc < 0) || (rc == 0x80) ? ("fail") : ("OK"));
    
    if (rc != 0)
    {
      if (rc == 0x80)
      {
        rt_kprintf("QoS(%d) config err!", qos);
      }
  ...
  while (1)
  {
    ...
    if(time_diff >= c->keepalive_interval) {
      ...
      if (rc != 0)
      {
        rt_kprintf("[%d] send ping rc: %d ", rt_tick_get(), rc);
        goto __mqtt_disconnect;
      }
      timeout.tv_sec = 10;
      c->keepalive_counter++;
      if(c->keepalive_counter >= c->keepalive_count) {
        rt_kprintf("[%d] can't recv Ping Response");
        goto __mqtt_disconnect;
      }
    } else {
...
int paho_mqtt_start(mqtt_client *client, rt_uint32_t stack_size, rt_uint8_t  priority)
{
  ...
  if (client->mqtt_lock == RT_NULL) {
    rt_kprintf("Create mqtt mutex error.");
    return PAHO_FAILURE;
  }
  ...
  if (client->msg_queue == RT_NULL) {
    rt_kprintf("Create mqtt msg pool error.");
    return PAHO_FAILURE;
  }
...
int paho_mqtt_subscribe(mqtt_client *client, enum QoS qos, const char *topic, subscribe_cb callback)
{
  ...
  for (i = 0; i < MAX_MESSAGE_HANDLERS ; ++i)
  {
    ...
    {
      rt_kprintf("MQTT client topic(%s) is already subscribed.", topic);
      return PAHO_SUCCESS;
    }
  }
  ...
  for (i = 0; i < MAX_MESSAGE_HANDLERS; ++i)
  {
    ...
    if (length <= 0)
    {
      rt_kprintf("Subscribe #%d %s failed!", i, topic);
    ...
    if(rt_mq_recv(client->msg_queue, &msg, sizeof(mqtt_message_ack), 
      ...
      if(msg.msg_type == SUBACK) {
        ...
        rt_kprintf("Subscribe #%d %s OK!", i, topic);
      } else {
        rt_kprintf("Subscribe #%d %s suback error!", i, topic);
      }
    } else {
      rt_kprintf("Subscribe #%d %s timeout!", i, topic);
    }
    goto __subscribe_exit;
  }
  ...
  if (i >= MAX_MESSAGE_HANDLERS) {
    rt_kprintf("Subscribe MAX_MESSAGE_HANDLERS size(%d) is not enough!", MAX_MESSAGE_HANDLERS);
    rc = PAHO_FAILURE;
  }
...
int paho_mqtt_unsubscribe(mqtt_client *client, const char *topic)
{
  ...
  for (i = 0; i < MAX_MESSAGE_HANDLERS; ++i)
  {
    ...
    if (length <= 0)
    {
      rt_kprintf("Unubscribe #%d %s failed!", i, topic);
      client->isconnected = 0;
      mqtt_client_unlock(client);
    ...
    if(rt_mq_recv(client->msg_queue, &msg, sizeof(mqtt_message_ack), 
      ...
      if(msg.msg_type == UNSUBACK) {
        ...
        rt_kprintf("Unsubscribe #%d %s OK!", i, topic);
      } else {
        rt_kprintf("Unsubscribe #%d %s unsuback error!", i, topic);
      }
    } else {
      rt_kprintf("Unsubscribe #%d %s timeout!", i, topic);
    }
  ...
  if (i >= MAX_MESSAGE_HANDLERS)
  {
    rt_kprintf("Unsubscribe topic(%s) is not exist!", topic);
    rc = PAHO_FAILURE;
  }
...
int paho_mqtt_control(mqtt_client *client, int cmd, void *arg)
{
  ...
  default:
    rt_kprintf("Input control commoand(%d) error.", cmd);
    break;
  }
