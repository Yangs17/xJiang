...
#define MBEDTLS_WEB_SERVER  "192.168.2.242"
#define MBEDTLS_WEB_PORT    "8888"
...
static void mbedtls_client_entry(void *parament)
{
    ...
    rt_kprintf("MbedTLS test sample, len = %d !\r\n",sizeof(MbedTLSSession));
...
int mbedtls_client_start(void)
{
    ...
    tid = rt_thread_create("tls_c", mbedtls_client_entry, RT_NULL, 6 * 1024, 15, 20);
    if (tid)
    {
