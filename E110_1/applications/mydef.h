...
typedef struct TCMD_112_INFO{
	/* 服务器下发包 */
	//rt_uint16_t mark ;       //1: 辅机信息表  2: IO表信息   3：log 表信息
	//rt_uint32_t recvPackLen; //接收了多少长度

	/* 客户端上发包 Abox 有处理 Cbox 未做处理*/
	rt_uint16_t mark ;         //1: 辅机信息表  2: IO表信息   3：log 表信息
	rt_uint32_t totalPackLen;  //表总长 (服务器不考虑)
	
	rt_uint16_t  pcaklen;      //当前发送的长度
	//char         *data;        //[packlen]; //表的内容
