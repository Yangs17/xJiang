...
uint32_t FLASH_If_Erase(uint32_t StartSector)
{
	  ...
	  StartSector = GetSector(StartSector);  //get start Serial number
	  EndSector = GetSector(USER_FLASH_END_ADDRESS);


	  //erase FLASH
  for (i = StartSector; i < EndSector; i += 8)  //FLASH serial number £¬reference  "FLASH Sector"¡£
  {
